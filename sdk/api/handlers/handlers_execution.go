...
func (h *BaseAPIHandler) executeWithAuthManagerFormats(ctx context.Context, entryProtocol, exitProtocol, modelName string, rawJSON []byte, alt string, allowImageModel bool, execOptions modelExecutionOptions) ([]byte, http.Header, *interfaces.ErrorMessage) {
	group, accessErr := modelGroupForRequest(ctx, modelName, execOptions.ModelGroupResolved)
	if accessErr != nil {
	...
	if group != nil {
		...
		for _, candidate := range modelGroupCandidates(group) {
			...
			body, headers, errMsg := h.executeWithAuthManagerFormats(ctx, entryProtocol, exitProtocol, candidate, rawJSON, alt, allowImageModel, candidateOptions)
			if errMsg == nil {
				return body, headers, nil
			}
			lastErr = errMsg
			if !shouldModelGroupFailover(errMsg.Error) {
...
func (h *BaseAPIHandler) executeCountWithAuthManager(ctx context.Context, handlerType, modelName string, rawJSON []byte, alt string, execOptions modelExecutionOptions) ([]byte, http.Header, *interfaces.ErrorMessage) {
	...
	if group != nil {
		...
		for _, candidate := range modelGroupCandidates(group) {
			candidateOptions := execOptions
			candidateOptions.ModelGroupResolved = true
			body, headers, errMsg := h.executeCountWithAuthManager(ctx, handlerType, candidate, rawJSON, alt, candidateOptions)
			if errMsg == nil {
				return body, headers, nil
			}
			lastErr = errMsg
