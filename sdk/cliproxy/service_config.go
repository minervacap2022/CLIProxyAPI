...
func (s *Service) applyConfigRuntime(ctx context.Context, commit configCommit, synthesizeConfigAuths bool) bool {
	...
	if commit.previous == nil || !reflect.DeepEqual(commit.previous.Warmup, cfg.Warmup) {
		if errWarmup := (&warmupAdapter{service: s}).Reload(); errWarmup != nil {
			log.WithError(errWarmup).Warn("warmup scheduler reload failed")
		}
	}
	s.reloadUsageServices(ctx, commit.previous, cfg)
	if errContext := ctx.Err(); errContext != nil {
		return false
	}
