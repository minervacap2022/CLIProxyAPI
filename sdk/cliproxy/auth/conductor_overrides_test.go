...
func TestManager_MarkResult_ClaudeExactShortResetLocksAccount(t *testing.T) {
	m := NewManager(nil, nil, nil)
	auth := &Auth{
		ID:       "auth-short-account-quota",
		Provider: "claude",
		ModelStates: map[string]*ModelState{
			"claude-opus-4-5":   {Status: StatusActive},
			"claude-sonnet-4-6": {Status: StatusActive},
	...
	if !updated.Unavailable || updated.NextRetryAfter.IsZero() {
		t.Fatalf("auth cooldown = unavailable:%v next:%v, want active account cooldown", updated.Unavailable, updated.NextRetryAfter)
	}
	if state := updated.ModelStates["claude-sonnet-4-6"]; state == nil || !state.Unavailable {
		t.Fatalf("unaffected model state = %+v, want account-wide cooldown", state)
	}
}
