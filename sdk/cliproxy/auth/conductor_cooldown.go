...
func (m *Manager) MarkResult(ctx context.Context, result Result) {
	...
	if auth, ok := m.auths[result.AuthID]; ok && auth != nil {
				...
				if !isRequestScopedResultError(result.Error) {
					...
					if isModelSupportResultError(result.Error) {
						...
						case 429:
							...
							accountQuota := result.AccountQuota || result.RetryAfter != nil && *result.RetryAfter > 10*time.Minute
							if !disableCooling {
							...
							if accountQuota {
								authQuotaReason := "quota"
								if result.AccountQuota {
									authQuotaReason = "account_quota"
								}
								for _, modelState := range auth.ModelStates {
									...
									modelState.Status = StatusError
									modelState.NextRetryAfter = next
									modelState.Quota = QuotaState{Exceeded: true, Reason: "quota", NextRecoverAt: next, BackoffLevel: modelState.Quota.BackoffLevel}
								}
								auth.Unavailable = true
								auth.NextRetryAfter = next
								auth.Quota = QuotaState{Exceeded: true, Reason: authQuotaReason, NextRecoverAt: next}
							}
							state.NextRetryAfter = next
							state.Quota = QuotaState{
								Exceeded:      true,
		...
		if errPersist := m.persist(ctx, auth); errPersist != nil {
			logEntryWithRequestID(ctx).WithField("auth_id", auth.ID).Warnf("auth persist failed: %v", errPersist)
		}
...
func accountQuotaFromError(err error) bool {
	if err == nil {
		return false
	}
	...
	var provider accountQuotaProvider
	return errors.As(err, &provider) && provider != nil && provider.AccountQuota()
}
