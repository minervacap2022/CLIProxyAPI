...
func (m *Manager) tryRefreshAfterUnauthorized(ctx context.Context, auth *Auth, execErr error, alreadyTried bool) (*Auth, bool) {
	...
	if errRefresh != nil || refreshed == nil {
		log.Warnf("credential refresh before fallback failed for %s (%s): %v", auth.Provider, auth.ID, errRefresh)
		return auth, false
	}
...
func (m *Manager) refreshAuth(ctx context.Context, id string) {
	_, errRefresh := m.refreshAuthForRequest(ctx, id, "")
	if errRefresh != nil && !errors.Is(errRefresh, context.Canceled) {
		log.WithError(errRefresh).WithField("auth_id", id).Warn("scheduled auth refresh failed")
	}
}
...
func (m *Manager) refreshAuthForRequest(ctx context.Context, id, failedAccessToken string) (*Auth, error) {
	...
	if errUpdate != nil {
		log.WithError(errUpdate).WithFields(log.Fields{"provider": auth.Provider, "auth_id": auth.ID}).Warn("auth refresh update failed")
	}
	if saved != nil {
		return saved, nil
	}
