...
func TestManager_RestoreCooldownStates(t *testing.T) {
	...
	if _, errRegister := manager.Register(WithSkipPersist(context.Background()), &Auth{ID: "auth-2", Provider: "xai"}); errRegister != nil {
		t.Fatalf("Register() returned error: %v", errRegister)
	}
	...
	authLevel, ok := manager.GetByID("auth-2")
	if !ok {
		t.Fatal("restored auth-level cooldown was not found")
	}
	if authLevel.Quota.Reason != "account_quota" {
		t.Fatalf("restored auth quota reason = %q, want %q", authLevel.Quota.Reason, "account_quota")
	}
	if got := store.saveCount.Load(); got != 1 {
