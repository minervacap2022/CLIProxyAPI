						...
						case 429:
							...
							if !disableCooling {
								if result.RetryAfter != nil {
									...
									// Account-level lock: only when cooldown is long enough to
									// indicate account-scoped quota exhaustion (Anthropic or Doubao
									// AccountQuotaExceeded). Short durations (<=10 min) are per-model
									// burst/rate-limit signals (Doubao RequestBurstTooFast).
