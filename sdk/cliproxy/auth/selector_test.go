...
func TestIsAuthBlockedForModel_AuthCooldownBlocksUnseenModel(t *testing.T) {
	t.Parallel()
	...
	auth := &Auth{
		...
		Unavailable:    true,
		NextRetryAfter: next,
		Quota:          QuotaState{Exceeded: true, Reason: "account_quota", NextRecoverAt: next},
		ModelStates: map[string]*ModelState{
			"known-model": {Status: StatusActive},
		},
	}

	blocked, reason, gotNext := isAuthBlockedForModel(auth, "unseen-model", now)
	if !blocked {
...
func TestIsAuthBlockedForModel_AggregatedKnownModelCooldownDoesNotBlockUnseenModel(t *testing.T) {
	t.Parallel()

	now := time.Now()
	next := now.Add(30 * time.Minute)
	auth := &Auth{
		ID:             "aggregated-model-cooldown",
		Unavailable:    true,
		NextRetryAfter: next,
