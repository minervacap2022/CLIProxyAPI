...
func (s *Service) Run(ctx context.Context) error {
	...
	if s.server != nil && s.server.ManagementHandler() != nil {
		s.server.ManagementHandler().SetWarmupController(&warmupAdapter{service: s})
	}
	if s.cfg.Warmup.Enabled {
		if errWarmup := (&warmupAdapter{service: s}).Reload(); errWarmup != nil {
			log.WithError(errWarmup).Warn("warmup scheduler disabled")
		}
	}
	if homeEnabled {
...
func (s *Service) Shutdown(ctx context.Context) error {
	...
	s.shutdownOnce.Do(func() {
		...
		s.warmupMu.Lock()
		warmupScheduler := s.warmupScheduler
		s.warmupScheduler = nil
		s.warmupMu.Unlock()
		...
		if errUsage := usage.StopDefaultAndWait(ctx); errUsage != nil {
			log.WithError(errUsage).Warn("usage manager queue drain failed")
			if shutdownErr == nil {
