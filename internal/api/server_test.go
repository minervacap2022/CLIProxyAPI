...
func TestDirectCodexRoutesEnforceModelAccess(t *testing.T) {
	server := newTestServer(t)
	server.cfg.APIKeyConfigs = []proxyconfig.APIKeyConfig{{Key: "test-key", AllowedModels: []string{"allowed-model"}}}
	server.rebuildKeyConfigIndexes(server.cfg)
	...
	tests := []struct {
		...
		{path: "/v1/alpha/search", contentType: "application/json", body: `{"model":"denied-model"}`},
		{path: "/backend-api/codex/alpha/search", contentType: "application/json", body: `{"model":"denied-model"}`},
		{path: "/v1/live", contentType: "application/json", body: `{"model":"denied-model"}`},
		{path: "/v1/realtime/calls", contentType: "application/json", body: `{"model":"denied-model"}`},
	}
	for _, test := range tests {
		t.Run(test.path, func(t *testing.T) {
			...
			if !strings.Contains(rr.Body.String(), `model \"denied-model\" is not allowed`) {
				t.Fatalf("body = %s, want normal access error", rr.Body.String())
			}
		})
	}
}
...
func TestManagementUsageRequiresManagementAuthAndPopsArray(t *testing.T) {
	...
	// Klik fork preserves the legacy in-memory /usage endpoint (restored after
	// the upstream removal), so it returns 200 with a usage snapshot rather
	// than the upstream 404.
