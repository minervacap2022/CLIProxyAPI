...
func (s *Server) setupRoutes() {
	...
	v1.Use(s.keyConfigMiddleware())
	{
		...
		v1.POST("/alpha/search", s.enforceDirectModelAccess(s.codexAlphaSearchModel, s.codexAlphaSearch))
		v1.POST("/live", s.enforceDirectModelAccess(s.codexLiveModel, s.codexLiveHandler.Handle))
		v1.GET("/live/:call_id", s.codexLiveHandler.HandleSideband)
		v1.POST("/realtime/calls", s.enforceDirectModelAccess(s.codexLiveModel, s.codexLiveHandler.Handle))
	...
	openaiV1.Use(s.keyConfigMiddleware())
	{
	...
	codexDirect.Use(s.keyConfigMiddleware())
	{
		codexDirect.GET("/responses", openaiResponsesHandlers.ResponsesWebsocket)
		codexDirect.POST("/responses", openaiResponsesHandlers.Responses)
		codexDirect.POST("/responses/compact", openaiResponsesHandlers.Compact)
		codexDirect.POST("/alpha/search", s.enforceDirectModelAccess(s.codexAlphaSearchModel, s.codexAlphaSearch))
	}
...
func (s *Server) enforceDirectModelAccess(modelFromRequest func(*gin.Context) (string, error), next gin.HandlerFunc) gin.HandlerFunc {
	return func(c *gin.Context) {
		model, err := modelFromRequest(c)
		if err != nil {
			s.handlers.WriteErrorResponse(c, &interfaces.ErrorMessage{StatusCode: http.StatusBadRequest, Error: err})
			return
		}
		keyConfigValue, _ := c.Get("apiKeyConfig")
		keyConfig, _ := keyConfigValue.(*config.APIKeyConfig)
		if err = modelgroup.CheckModelAccess(keyConfig, model); err != nil {
			s.handlers.WriteErrorResponse(c, &interfaces.ErrorMessage{StatusCode: http.StatusForbidden, Error: err})
			return
		}
		next(c)
	}
...
func directRequestBody(c *gin.Context) ([]byte, error) {
...
func (s *Server) codexAlphaSearchModel(c *gin.Context) (string, error) {
	body, err := directRequestBody(c)
	if err != nil {
		return "", err
	}
	var routing struct {
	...
	return strings.TrimSpace(routing.Model), nil
}

func (s *Server) codexLiveModel(c *gin.Context) (string, error) {
	body, err := directRequestBody(c)
	if err != nil {
		return "", err
	}
	return codexlive.ModelFromRequest(body, c.GetHeader("Content-Type"))
}
...
func (s *Server) unifiedModelsHandler(openaiHandler *openai.OpenAIAPIHandler, claudeHandler *claude.ClaudeCodeAPIHandler) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if isAnthropicModelsRequest(c) {
			s.serveClaudeModelsWithGroups(c, claudeHandler)
		} else {
			s.serveOpenAIModelsWithGroups(c, openaiHandler)
		}
	}
}
