...
func SaveConfigPreserveComments(configFile string, cfg *Config) error {
	...
	for _, keyConfig := range cfg.APIKeyConfigs {
		policyKeys[strings.TrimSpace(keyConfig.Key)] = struct{}{}
	}
	persistCfg.APIKeys = persistCfg.APIKeys[:0]
	for _, key := range cfg.APIKeys {
		if _, policy := policyKeys[strings.TrimSpace(key)]; !policy {
			persistCfg.APIKeys = append(persistCfg.APIKeys, key)
		}
	}
