...
func TestClaudeExactResetMarksShortAccountQuota(t *testing.T) {
	reset := time.Now().Add(5 * time.Minute).Unix()
	header := http.Header{"Anthropic-Ratelimit-Unified-Reset": []string{fmt.Sprint(reset)}}

	err := newClaudeStatusError(http.StatusTooManyRequests, "quota", header)
	if !err.AccountQuota() {
		t.Fatal("exact Anthropic reset must mark account quota")
	}
	if err.RetryAfter() == nil || *err.RetryAfter() <= 0 || *err.RetryAfter() > 10*time.Minute {
		t.Fatalf("RetryAfter = %v, want positive duration <= 10m", err.RetryAfter())
	}
}
